    ...
    **kwargs,
    ...
    if frame.group_by:
        if frame.order_by:
            ...
            # The implementaiton below is mostly working. Need to figure out a few Dask quirks.
            # sorted_df, grouping_keys, ordering_keys = compute_sorted_frame(
            #     df = root_data,
            ...
            # source = sorted_df.groupby(group_by_cols, sort=True, group_keys=False)
        else:
            if len(grouping_keys) == 1 and isinstance(grouping_keys[0], dd.Series):
                # Dask will raise an exception about not supporting multiple Series in group by key
                # even if it is passed a length 1 list of Series.
