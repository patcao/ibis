...
from ibis.backends.dask.aggcontext import AggregationContext, window_agg_udf
...
def test_lead(t, df, row_offset, default, row_window):
    ...
    expected.name = f"Lead(dup_strings, {row_offset}, {default})"
...
def test_lag(t, df, row_offset, default, row_window):
    ...
    if default is not ibis.NA:
        expected = expected.fillna(execute(default.op()))
    expected.name = f"Lag(dup_strings, {row_offset}, {default})"
    tm.assert_series_equal(result, expected)
...
def test_lead_delta(t, df, range_offset, default, range_window):
    ...
    if default is not ibis.NA:
        expected = expected.fillna(execute(default.op()))
    expected.name = f"Lead(dup_strings, {range_offset.get_name()}, {default})"
    tm.assert_series_equal(result, expected)
...
def test_lag_delta(t, df, range_offset, default, range_window):
    ...
    if default is not ibis.NA:
        expected = expected.fillna(execute(default.op()))
    expected.name = f"Lag(dup_strings, {range_offset.get_name()}, {default})"
    tm.assert_series_equal(result, expected)
...
def test_first(t, df):
    ...
    result = expr.execute()
    assert result == df.dup_strings.compute().iloc[0]


def test_last(t, df):
    expr = t.dup_strings.last()
    result = expr.execute()
    assert result == df.dup_strings.compute().iloc[-1]
...
def test_window_grouping_key_has_scope(t, df):
    ...
    expected.name = 'Mean(plain_int64)'
